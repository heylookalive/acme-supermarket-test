...
export const productsLoad = () => async dispatch => {
  ...
  try {
    const products = await productsLoadFromAPI();
    dispatch({ type: PRODUCTS_LOAD_SUCCESS, products });
  } catch (error) {
    dispatch({ type: PRODUCTS_LOAD_ERROR, error });
  }
};

export const pricingRulesLoad = () => async dispatch => {
  dispatch({ type: PRICING_RULES_LOAD_BEGIN });

  try {
    const pricingRules = await pricingRulesLoadFromAPI();
    dispatch({ type: PRICING_RULES_LOAD_SUCCESS, pricingRules });
  } catch (error) {
    dispatch({ type: PRICING_RULES_LOAD_SUCCESS, error });
  }
};
